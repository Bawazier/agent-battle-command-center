import { BufferGeometry, BoxGeometry, ConeGeometry, CylinderGeometry, TorusGeometry } from 'three';
 ...
 * Returns a Group containing all geometry pieces for the building.
 */

/** Small hut/shed - complexity 1-2 */
function createHutGeometry(): BufferGeometry[] {
  const geometries: BufferGeometry[] = [];

  // Base box
  const base = new BoxGeometry(1.2, 0.8, 1.2);
  base.translate(0, 0.4, 0);
  geometries.push(base);

  // Roof (pyramid via cone with 4 sides)
  const roof = new ConeGeometry(0.9, 0.6, 4);
...
function createBarracksGeometry(): BufferGeometry[] {
  const geometries: BufferGeometry[] = [];

  // Main building - elongated
  const main = new BoxGeometry(2, 1.2, 1);
  main.translate(0, 0.6, 0);
  geometries.push(main);

  // Roof ridge
  const roof = new CylinderGeometry(0, 0.6, 0.5, 4);
  ...
  // Door frame
  const door = new BoxGeometry(0.3, 0.6, 0.05);
...
function createFortressGeometry(): BufferGeometry[] {
  const geometries: BufferGeometry[] = [];

  // Main structure
  const base = new BoxGeometry(2, 1.5, 2);
  ...
  for (const [px, , pz] of pillarPositions) {
    const pillar = new CylinderGeometry(0.12, 0.12, 2, 6);
    pillar.translate(px, 1, pz);
    geometries.push(pillar);
  }

  // Top battlement ring
  const ring = new TorusGeometry(1.1, 0.08, 4, 4);
...
function createCitadelGeometry(): BufferGeometry[] {
  const geometries: BufferGeometry[] = [];

  // Central keep
  const keep = new BoxGeometry(1.5, 2.5, 1.5);
  keep.translate(0, 1.25, 0);
  geometries.push(keep);

  // Outer walls
  const wall = new BoxGeometry(3, 1.2, 3);
  ...
  for (const [tx, , tz] of towerPositions) {
    const tower = new CylinderGeometry(0.3, 0.35, 2, 8);
    tower.translate(tx, 1, tz);
    geometries.push(tower);

    // Tower top
    const cap = new ConeGeometry(0.35, 0.5, 8);
    cap.translate(tx, 2.25, tz);
    geometries.push(cap);
  }

  // Central spire
  const spire = new ConeGeometry(0.2, 1, 6);
...
function createCastleGeometry(): BufferGeometry[] {
  ...
  // Central tower (tall)
  const central = new BoxGeometry(1.2, 3.5, 1.2);
  central.translate(0, 1.75, 0);
  geometries.push(central);

  // Central spire
  const spire = new ConeGeometry(0.8, 1.5, 6);
  spire.translate(0, 4.25, 0);
  geometries.push(spire);

  // Inner wall
  const innerWall = new BoxGeometry(3, 1.5, 3);
  innerWall.translate(0, 0.75, 0);
  geometries.push(innerWall);

  // Outer wall (lower)
  const outerWall = new BoxGeometry(4, 1, 4);
  ...
  for (let i = 0; i < towerPositions.length; i++) {
    ...
    const tower = new CylinderGeometry(radius, radius + 0.05, height, 8);
    tower.translate(tx, height / 2, tz);
    geometries.push(tower);

    if (isCorner) {
      const cap = new ConeGeometry(radius + 0.05, 0.6, 8);
  ...
  // Gate
  const gate = new BoxGeometry(0.6, 0.8, 0.1);
...
export function createBuildingGeometries(tier: BuildingTier): BufferGeometry[] {
  switch (tier) {
...
export function disposeGeometries(geometries: BufferGeometry[]): void {
