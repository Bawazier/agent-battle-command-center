...
/**
 * Ollama Stress Test - Qwen3:8B with Thinking (1-8)
 *
 * Tests Qwen3:8B (with thinking enabled) on 20 tasks with increasing complexity.
 * Comparison test against qwen2.5-coder:7b baseline (100% C1-C8).
 *
 * Model: qwen3:8b-think (custom Modelfile, 8K ctx, temp 0, thinking ON)
 * Key difference: Qwen3 uses internal chain-of-thought before responding.
 *
...
const MODEL = 'qwen3:8b-think'; // Qwen3 8B with thinking enabled

const API_BASE = 'http://localhost:3001/api';
const AGENTS_BASE = 'http://localhost:8000';
const API_KEY = process.env.API_KEY || 'ceb3e905f7b1b5e899645c6ec467ca34';
const TASK_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes per task (thinking needs more time)
const REST_DELAY_MS = 3000; // 3 seconds rest between tasks
const RESET_EVERY_N_TASKS = 2; // Reset agent memory every N tasks (thinking bloats context fast)
...
async function main() {
  ...
  console.log('🧠 QWEN3:8B THINKING STRESS TEST - GRADUATED COMPLEXITY (1-8)');
  console.log('═'.repeat(70));
  console.log('Goal: Test Qwen3 8B with thinking vs qwen2.5-coder:7b baseline');
  console.log(`Tasks: 20 | Complexity range: 1-8 | Model: ${MODEL}`);
  ...
  const fs = require('fs');
  fs.writeFileSync(
    'scripts/qwen3-8b-think-stress-results.json',
    JSON.stringify({
      ...results,
      totalDuration,
      successRate,
  ...
  console.log('\n💾 Results saved to scripts/qwen3-8b-think-stress-results.json');
